...
export default function MathLevelsPage() {
    ...
    const highestCompletedMathLevel = userProgress
      ?.filter(p => p.moduleId.startsWith('math-') && p.completed)
      .map(p => parseInt(p.moduleId.split('-')[1], 10))
      .reduce((max, level) => (level > max ? level : max), 0) || 0;

    const unlockedLevels = highestCompletedMathLevel + 1;
        ...
        <div className="container mx-auto max-w-4xl py-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {mathLevels.map(level => {
                    ...
                    const progress = userProgress?.find(p => p.moduleId === `math-${level.level}`);
