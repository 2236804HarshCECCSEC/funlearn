...
import { scienceLevels } from '@/lib/science-questions';
import { Lock, Unlock, CheckCircle } from 'lucide-react';
import Link from 'next/link';
import { useFirebase, useCollection, useMemoFirebase } from '@/firebase';
import { collection } from 'firebase/firestore';

export default function ScienceLevelsPage() {
    ...
    const highestCompletedScienceLevel = userProgress
      ?.filter(p => p.moduleId.startsWith('science-') && p.completed)
      .map(p => parseInt(p.moduleId.split('-')[1], 10))
      .reduce((max, level) => (level > max ? level : max), 0) || 0;

    const unlockedLevels = highestCompletedScienceLevel + 1;
    
    if (isLoading) {
      return (
        <div className="container mx-auto max-w-4xl py-8">
            <div className="text-center mb-8">
                <h1 className="text-4xl font-bold font-headline">Science Levels</h1>
        ...
        <div className="container mx-auto max-w-4xl py-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {scienceLevels.map(level => {
                    ...
                    const progress = userProgress?.find(p => p.moduleId === `science-${level.level}`);
                    const isCompleted = progress?.completed;

                    return (
                        <Link key={level.level} href={isUnlocked ? `/science/levels/${level.level}` : '#'} className={!isUnlocked ? 'pointer-events-none' : ''}>
